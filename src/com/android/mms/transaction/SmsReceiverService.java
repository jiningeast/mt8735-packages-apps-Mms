...
public class SmsReceiverService extends Service implements IMissedSmsReceiverHost {
    ...
    /// M: new members
    private IDisplayClassZeroMessageExt mDisplayClassZeroPlugin = null;
    private IMissedSmsReceiverExt mMissedSmsReceiverPlugin = null;
...
//        }
    ...
    public synchronized void sendFirstQueuedMessage() {
        ...
        String selection = Sms.SUBSCRIPTION_ID + " NOT IN " + "(" + getNoServiceSimString() + ")";
    ...
    private void handleSmsReceived(Intent intent, int error) {
        ...
        if (MmsConfig.isMissedSmsReceiverEnable()) {
            uploadFlag = intent.getIntExtra(IConcatenatedSmsFwkExt.UPLOAD_FLAG_TAG,
        ...
        try {
            ...
            if (msgs != null && msgs[0].getMessageClass() != SmsMessage.MessageClass.CLASS_0
                && !msgs[0].isReplace() && MmsConfig.isMissedSmsReceiverEnable()
                && uploadFlag == IConcatenatedSmsFwkExt.UPLOAD_FLAG_UPDATE) {
                messageUri = mMissedSmsReceiverPlugin.updateMissedSms(this, msgs, error);
            /// @}
            } else {
...
//        }
    ...
    private void displayClassZeroMessage(Context context, SmsMessage[] msgs, String format) {
        ...
        /// M: add for OP09 @{
        if (MmsConfig.isClassZeroModelShowLatestEnable()) {
            smsDialogIntent = mDisplayClassZeroPlugin.setLaunchMode(smsDialogIntent);
        }
        /// @}
    ...
    private void initPlugin(Context context) {
        ...
        if(mDisplayClassZeroPlugin == null){
            mDisplayClassZeroPlugin = new DefaultDisplayClassZeroMessageExt(context.getApplicationContext());
            MmsLog.d(TAG, "default mDisplayClassZeroPlugin = " + mDisplayClassZeroPlugin);
        }

        mMissedSmsReceiverPlugin = (IMissedSmsReceiverExt) MPlugin.createInstance(
                IMissedSmsReceiverExt.class.getName(),context.getApplicationContext());
        MmsLog.d(TAG, "operator mMissedSmsReceiverPlugin = " + mMissedSmsReceiverPlugin);
        if(mMissedSmsReceiverPlugin == null){
            mMissedSmsReceiverPlugin = new DefaultMissedSmsReceiverExt(context.getApplicationContext());
