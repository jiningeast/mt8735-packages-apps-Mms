...
public class MmsPlayerActivity extends Activity implements Contact.UpdateListener, IMmsTextSizeAdjustHost {
    ...
    public static MmsPlayerActivity sCurrentActivity;
//    private static CellConnMgr mCellMgr = null;//csw
        ...
        if (mDirMode) {
            ...
            if (fileAttachmentCount > 0) {
                ...
                IMmsAttachmentEnhanceExt attachEnhancePlugin =
                        (IMmsAttachmentEnhanceExt) MmsPluginManager.getMmsPluginObject(MmsPluginManager.MMS_PLUGIN_TYPE_MMS_ATTACHMENT_ENHANCE);
                if (attachEnhancePlugin != null) {
                    attachEnhancePlugin.setAttachmentName(mAttachName, fileAttachmentCount);
                }
                /// @}
            }
        }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if (mDirMode) {
            ...
            if (mIsSmsEnabled) {
                ...
                } else if ((mMsgBox == Mms.MESSAGE_BOX_FAILED) && mShowResend/*send fail*/) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
     ...
     IMmsAttachmentEnhanceExt attachEnhancePlugin = (IMmsAttachmentEnhanceExt) MmsPluginManager
        ...
        case MENU_RESEND:
            ...
            if (!MmsConfig.isSupportAttachEnhance()) {
                MmsLog.d(TAG, "ResendMessage(): isSupportAttachmentEnhance is false");
                for (FileAttachmentModel fileAttachment : mSlideshow.getAttachFiles()) {
                    attachmentSize += fileAttachment.getAttachSize();
                }
            }
            messageSize = mSlideshow.getCurrentSlideshowSize() + attachmentSize;
    ...
    private void parseMsgUri() {
        ...
        try {
            ...
            if (mMsgBox == Mms.MESSAGE_BOX_FAILED) {
                checkSendFail();
            }
    ...
    private void initPlugin(Context context) {
        mMmsTextSizeAdjustPlugin = (IMmsTextSizeAdjustExt) MmsPluginManager.getMmsPluginObject(
